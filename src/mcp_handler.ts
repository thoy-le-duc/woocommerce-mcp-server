...
import type {
  McpHandlerParams,
  WcApiCredentials,
  // SimpleAxiosError, // Pas utilisé directement
  // WordPressErrorData, // Pas utilisé directement
} from './types.js'; // Extension .js requise
 ...
 */
export async function handleMcpRequest(method: string, params: McpHandlerParams = {}): Promise<any> {
  try { // <--- Début du TRY principal
    // 1. Determine Credentials: Start with an empty object, then populate conditionally.
    const creds: WcApiCredentials = {}; // Start empty, satisfies optional properties
    ...
    if (siteUrlValue) creds.siteUrl = siteUrlValue;
    if (usernameValue) creds.username = usernameValue;
    if (passwordValue) creds.password = passwordValue;
    if (consumerKeyValue) creds.consumerKey = consumerKeyValue;
    if (consumerSecretValue) creds.consumerSecret = consumerSecretValue;
    ...
    switch (method) {
        ...
        // === WordPress Post Meta ===
        case 'get_post_meta':
            if (!params.postId) throw new Error('postId is required for get_post_meta');
        ...
        case 'update_post_meta': // WP typically handles create/update via main post update
            if (!params.postId) throw new Error('postId is required for create/update_post_meta');
            if (!params.metaKey) throw new Error('metaKey is required for create/update_post_meta');
            if (params.metaValue === undefined) throw new Error('metaValue is required for create/update_post_meta');
        ...
        case 'delete_post_meta':
            if (!params.postId) throw new Error('postId is required for delete_post_meta');
            if (!params.metaKey) throw new Error('metaKey is required for delete_post_meta');
        ...
        case 'get_product':
            if (!params.productId) throw new Error('productId is required for get_product');
            return (await client.get(`/products/${params.productId}`)).data;
        case 'create_product':
            if (!params.productData) throw new Error('productData is required for create_product');
            return (await client.post('/products', params.productData)).data;
        case 'update_product':
            if (!params.productId) throw new Error('productId is required for update_product');
            if (!params.productData) throw new Error('productData is required for update_product');
            return (await client.put(`/products/${params.productId}`, params.productData)).data;
        case 'delete_product':
            if (!params.productId) throw new Error('productId is required for delete_product');
        ...
        case 'get_order':
            if (!params.orderId) throw new Error('orderId is required for get_order');
            return (await client.get(`/orders/${params.orderId}`)).data;
        case 'create_order':
            if (!params.orderData) throw new Error('orderData is required for create_order');
            return (await client.post('/orders', params.orderData)).data;
        case 'update_order':
            if (!params.orderId) throw new Error('orderId is required for update_order');
            if (!params.orderData) throw new Error('orderData is required for update_order');
            return (await client.put(`/orders/${params.orderId}`, params.orderData)).data;
        case 'delete_order':
            if (!params.orderId) throw new Error('orderId is required for delete_order');
        ...
        case 'get_customer':
            if (!params.customerId) throw new Error('customerId is required for get_customer');
            return (await client.get(`/customers/${params.customerId}`)).data;
        case 'create_customer':
            if (!params.customerData) throw new Error('customerData is required for create_customer');
            return (await client.post('/customers', params.customerData)).data;
        case 'update_customer':
            if (!params.customerId) throw new Error('customerId is required for update_customer');
            if (!params.customerData) throw new Error('customerData is required for update_customer');
            return (await client.put(`/customers/${params.customerId}`, params.customerData)).data;
        case 'delete_customer':
            if (!params.customerId) throw new Error('customerId is required for delete_customer');
        ...
        case 'get_shipping_zone':
            if (!params.zoneId) throw new Error('zoneId is required for get_shipping_zone');
            return (await client.get(`/shipping/zones/${params.zoneId}`)).data;
        case 'create_shipping_zone':
            if (!params.zoneData) throw new Error('zoneData is required for create_shipping_zone');
            return (await client.post('/shipping/zones', params.zoneData)).data;
        case 'update_shipping_zone':
            if (!params.zoneId) throw new Error('zoneId is required for update_shipping_zone');
            if (!params.zoneData) throw new Error('zoneData is required for update_shipping_zone');
            return (await client.put(`/shipping/zones/${params.zoneId}`, params.zoneData)).data;
        case 'delete_shipping_zone':
            if (!params.zoneId) throw new Error('zoneId is required for delete_shipping_zone');
        ...
        case 'get_shipping_zone_methods':
            if (!params.zoneId) throw new Error('zoneId is required for get_shipping_zone_methods');
            return (await client.get(`/shipping/zones/${params.zoneId}/methods`)).data;
        case 'create_shipping_zone_method':
            if (!params.zoneId) throw new Error('zoneId is required for create_shipping_zone_method');
            if (!params.methodData) throw new Error('methodData is required for create_shipping_zone_method');
            return (await client.post(`/shipping/zones/${params.zoneId}/methods`, params.methodData)).data;
        case 'update_shipping_zone_method':
            if (!params.zoneId) throw new Error('zoneId is required for update_shipping_zone_method');
            if (!params.instanceId) throw new Error('instanceId is required for update_shipping_zone_method');
            if (!params.methodData) throw new Error('methodData is required for update_shipping_zone_method');
        ...
        case 'delete_shipping_zone_method':
            if (!params.zoneId) throw new Error('zoneId is required for delete_shipping_zone_method');
            if (!params.instanceId) throw new Error('instanceId is required for delete_shipping_zone_method');
        ...
        case 'get_shipping_zone_locations':
            if (!params.zoneId) throw new Error('zoneId is required for get_shipping_zone_locations');
        ...
        case 'update_shipping_zone_locations': // Usually PUT or POST with locations array
            if (!params.zoneId) throw new Error('zoneId is required for update_shipping_zone_locations');
            if (!params.locations) throw new Error('locations array is required for update_shipping_zone_locations');
        ...
        case 'create_tax_class':
            if (!params.taxClassData || !params.taxClassData.name) throw new Error('taxClassData with at least name is required for create_tax_class');
            return (await client.post('/taxes/classes', params.taxClassData)).data;
        case 'delete_tax_class':
            if (!params.slug) throw new Error('slug is required for delete_tax_class');
        ...
        case 'get_tax_rate':
            if (!params.rateId) throw new Error('rateId is required for get_tax_rate');
            return (await client.get(`/taxes/${params.rateId}`)).data;
        case 'create_tax_rate':
            if (!params.taxRateData) throw new Error('taxRateData is required for create_tax_rate');
            return (await client.post('/taxes', params.taxRateData)).data;
        case 'update_tax_rate':
            if (!params.rateId) throw new Error('rateId is required for update_tax_rate');
            if (!params.taxRateData) throw new Error('taxRateData is required for update_tax_rate');
            return (await client.put(`/taxes/${params.rateId}`, params.taxRateData)).data;
        case 'delete_tax_rate':
            if (!params.rateId) throw new Error('rateId is required for delete_tax_rate');
        ...
        case 'get_coupon':
            if (!params.couponId) throw new Error('couponId is required for get_coupon');
            return (await client.get(`/coupons/${params.couponId}`)).data;
        case 'create_coupon':
            if (!params.couponData) throw new Error('couponData is required for create_coupon');
            return (await client.post('/coupons', params.couponData)).data;
        case 'update_coupon':
            if (!params.couponId) throw new Error('couponId is required for update_coupon');
            if (!params.couponData) throw new Error('couponData is required for update_coupon');
            return (await client.put(`/coupons/${params.couponId}`, params.couponData)).data;
        case 'delete_coupon':
            if (!params.couponId) throw new Error('couponId is required for delete_coupon');
            return (await client.delete(`/coupons/${params.couponId}`, { params: { force: params.force ?? true } })).data;

        // === Order Notes ===
        case 'get_order_notes':
            if (!params.orderId) throw new Error('orderId is required for get_order_notes');
        ...
        case 'get_order_note':
            if (!params.orderId) throw new Error('orderId is required for get_order_note');
            if (!params.noteId) throw new Error('noteId is required for get_order_note');
            return (await client.get(`/orders/${params.orderId}/notes/${params.noteId}`)).data;
        case 'create_order_note':
            if (!params.orderId) throw new Error('orderId is required for create_order_note');
            if (!params.noteData || !params.noteData.note) throw new Error('noteData with note content is required for create_order_note');
            return (await client.post(`/orders/${params.orderId}/notes`, params.noteData)).data;
        case 'delete_order_note':
            if (!params.orderId) throw new Error('orderId is required for delete_order_note');
            if (!params.noteId) throw new Error('noteId is required for delete_order_note');
        ...
        case 'get_order_refunds':
            if (!params.orderId) throw new Error('orderId is required for get_order_refunds');
        ...
        case 'get_order_refund':
            if (!params.orderId) throw new Error('orderId is required for get_order_refund');
            if (!params.refundId) throw new Error('refundId is required for get_order_refund');
            return (await client.get(`/orders/${params.orderId}/refunds/${params.refundId}`)).data;
        case 'create_order_refund':
            if (!params.orderId) throw new Error('orderId is required for create_order_refund');
            if (!params.refundData) throw new Error('refundData is required for create_order_refund');
            return (await client.post(`/orders/${params.orderId}/refunds`, params.refundData)).data;
        case 'delete_order_refund':
            if (!params.orderId) throw new Error('orderId is required for delete_order_refund');
            if (!params.refundId) throw new Error('refundId is required for delete_order_refund');
        ...
        case 'get_product_variations':
            if (!params.productId) throw new Error('productId is required for get_product_variations');
        ...
        case 'get_product_variation':
            if (!params.productId) throw new Error('productId is required for get_product_variation');
            if (!params.variationId) throw new Error('variationId is required for get_product_variation');
            return (await client.get(`/products/${params.productId}/variations/${params.variationId}`)).data;
        case 'create_product_variation':
            if (!params.productId) throw new Error('productId is required for create_product_variation');
            if (!params.variationData) throw new Error('variationData is required for create_product_variation');
            return (await client.post(`/products/${params.productId}/variations`, params.variationData)).data;
        case 'update_product_variation':
            if (!params.productId) throw new Error('productId is required for update_product_variation');
            if (!params.variationId) throw new Error('variationId is required for update_product_variation');
            if (!params.variationData) throw new Error('variationData is required for update_product_variation');
        ...
        case 'delete_product_variation':
            if (!params.productId) throw new Error('productId is required for delete_product_variation');
            if (!params.variationId) throw new Error('variationId is required for delete_product_variation');
        ...
        case 'get_product_attribute':
            if (!params.attributeId) throw new Error('attributeId is required for get_product_attribute');
            return (await client.get(`/products/attributes/${params.attributeId}`)).data;
        case 'create_product_attribute':
            if (!params.attributeData) throw new Error('attributeData is required for create_product_attribute');
            return (await client.post('/products/attributes', params.attributeData)).data;
        case 'update_product_attribute':
            if (!params.attributeId) throw new Error('attributeId is required for update_product_attribute');
            if (!params.attributeData) throw new Error('attributeData is required for update_product_attribute');
            return (await client.put(`/products/attributes/${params.attributeId}`, params.attributeData)).data;
        case 'delete_product_attribute':
            if (!params.attributeId) throw new Error('attributeId is required for delete_product_attribute');
        ...
        case 'get_attribute_terms':
            if (!params.attributeId) throw new Error('attributeId is required for get_attribute_terms');
        ...
        case 'get_attribute_term':
            if (!params.attributeId) throw new Error('attributeId is required for get_attribute_term');
            if (!params.termId) throw new Error('termId is required for get_attribute_term');
            return (await client.get(`/products/attributes/${params.attributeId}/terms/${params.termId}`)).data;
        case 'create_attribute_term':
            if (!params.attributeId) throw new Error('attributeId is required for create_attribute_term');
            if (!params.termData) throw new Error('termData is required for create_attribute_term');
            return (await client.post(`/products/attributes/${params.attributeId}/terms`, params.termData)).data;
        case 'update_attribute_term':
            if (!params.attributeId) throw new Error('attributeId is required for update_attribute_term');
            if (!params.termId) throw new Error('termId is required for update_attribute_term');
            if (!params.termData) throw new Error('termData is required for update_attribute_term');
        ...
        case 'delete_attribute_term':
            if (!params.attributeId) throw new Error('attributeId is required for delete_attribute_term');
            if (!params.termId) throw new Error('termId is required for delete_attribute_term');
        ...
        case 'get_product_category':
            if (!params.categoryId) throw new Error('categoryId is required for get_product_category');
            return (await client.get(`/products/categories/${params.categoryId}`)).data;
        case 'create_product_category':
            if (!params.categoryData) throw new Error('categoryData is required for create_product_category');
            return (await client.post('/products/categories', params.categoryData)).data;
        case 'update_product_category':
            if (!params.categoryId) throw new Error('categoryId is required for update_product_category');
            if (!params.categoryData) throw new Error('categoryData is required for update_product_category');
            return (await client.put(`/products/categories/${params.categoryId}`, params.categoryData)).data;
        case 'delete_product_category':
            if (!params.categoryId) throw new Error('categoryId is required for delete_product_category');
        ...
        case 'get_product_tag':
             if (!params.tagId) throw new Error('tagId is required for get_product_tag');
             return (await client.get(`/products/tags/${params.tagId}`)).data;
        case 'create_product_tag':
             if (!params.tagData) throw new Error('tagData is required for create_product_tag');
             return (await client.post('/products/tags', params.tagData)).data;
        case 'update_product_tag':
             if (!params.tagId) throw new Error('tagId is required for update_product_tag');
             if (!params.tagData) throw new Error('tagData is required for update_product_tag');
             return (await client.put(`/products/tags/${params.tagId}`, params.tagData)).data;
        case 'delete_product_tag':
             if (!params.tagId) throw new Error('tagId is required for delete_product_tag');
        ...
        case 'get_product_review':
            if (!params.reviewId) throw new Error('reviewId is required for get_product_review');
        ...
        case 'update_product_review':
            if (!params.reviewId) throw new Error('reviewId is required for update_product_review');
            if (!params.reviewData) throw new Error('reviewData is required for update_product_review');
            return (await client.put(`/products/reviews/${params.reviewId}`, params.reviewData)).data;
        case 'delete_product_review':
            if (!params.reviewId) throw new Error('reviewId is required for delete_product_review');
        ...
        case 'get_payment_gateway':
            if (!params.gatewayId) throw new Error('gatewayId is required for get_payment_gateway');
            return (await client.get(`/payment_gateways/${params.gatewayId}`)).data;
        case 'update_payment_gateway':
            if (!params.gatewayId) throw new Error('gatewayId is required for update_payment_gateway');
            if (!params.gatewayData) throw new Error('gatewayData is required for update_payment_gateway');
        ...
        case 'get_setting_options': // Gets settings within a group
            if (!params.group) throw new Error('group (setting group id) is required for get_setting_options');
            return (await client.get(`/settings/${params.group}`)).data;
        case 'update_setting_option': // Updates a specific setting
            if (!params.group) throw new Error('group (setting group id) is required for update_setting_option');
            if (!params.id) throw new Error('id (setting id) is required for update_setting_option');
            if (!params.settingData || params.settingData.value === undefined) throw new Error('settingData with a value property is required for update_setting_option');
        ...
        case 'run_system_status_tool': // Usually PUT confirms/runs the tool
            if (!params.toolId) throw new Error('toolId is required for run_system_status_tool');
        ...
        case 'get_customer_meta': {
            ...
            let endpoint: string;
            if (method === 'get_product_meta') { id = params.productId; endpoint = `/products/${id}`; }
            else if (method === 'get_order_meta') { id = params.orderId; endpoint = `/orders/${id}`; }
            else { id = params.customerId; endpoint = `/customers/${id}`; } // customer meta

            if (!id) throw new Error(`ID (productId, orderId, or customerId) is required for ${method}`);

            const response = await client.get(endpoint);
            const metaData: { id?: number; key: string; value: any }[] = response.data.meta_data || [];
        ...
        case 'update_customer_meta': {
            let id: string | number | undefined;
            let endpoint: string;
            if (method.includes('product')) { id = params.productId; endpoint = `/products/${id}`; }
            else if (method.includes('order')) { id = params.orderId; endpoint = `/orders/${id}`; }
            else { id = params.customerId; endpoint = `/customers/${id}`; } // customer meta

            if (!id) throw new Error(`ID (productId, orderId, or customerId) is required for ${method}`);
            if (!params.metaKey) throw new Error(`metaKey is required for ${method}`);
            if (params.metaValue === undefined) throw new Error(`metaValue is required for ${method}`);

            // Get current meta_data
            const currentObject = (await client.get(endpoint)).data;
            let metaData: { id?: number; key: string; value: any }[] = currentObject.meta_data || [];
  ...
  catch (err: unknown) { // <--- Début du CATCH
