...
import type { JsonRpcRequest, JsonRpcResponse, JsonRpcErrorObject } from './types.js'; // Added .js
...
function sendResponse(response: JsonRpcResponse): void {
  try {
    ...
    const minimalError: JsonRpcResponse = {
        jsonrpc: '2.0',
...
rl.on('line', async (line) => {
  let request: JsonRpcRequest | null = null; // Initialise à null
  ...
  try {
    ...
    if (
      ...
      request = parsedData as JsonRpcRequest; // Type assertion après validation
    } else {
